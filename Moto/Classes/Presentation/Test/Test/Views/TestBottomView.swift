...
final class TestBottomView: GradientView {
    enum State {
        case confirm, submit, back, next, hidden
    }
...
// MARK: Public
extension TestBottomView {
    func setup(state: TestBottomView.State) {
        switch state {
        case .confirm:
            setupFullButton(text: "Question.Continue".localized)
        case .submit:
            setupFullButton(text: "Question.Submit".localized)
        case .back:
            setupFullButton(text: "Question.BackToStudying".localized)
        case .next:
            setupNextButton()
        case .hidden:
...
// MARK: Private
private extension TestBottomView {
    func initialize() {
